    ...
    <tr>
        ...
        <td>
            <link rel="StyleSheet" href="dtree.css" type="text/css" />
                        ...
                        function addMe(id)
                        {
                            $.get('ajax.categorycon.php?id='+id, function(data) {
                                $("#issue").val($("#issue").val()+data);
                            });
                        }

                        function locationurl()
                        {
                            var url = 'tickets.php?a=open&keyword='+document.getElementById('keyword').value;
//                            alert(url);
                            ...
                            <p><a href="javascript: d.openAll();">open all</a> | <a href="javascript: d.closeAll();">close all</a></p>
                                ...
                                while(list($classId,$name,$parentId)=db_fetch_row($class)) {
                                    ...
                                    while($row_cate = db_fetch_row($result_cate)) {
                                        ...
                                        $tempUrl="javascript:addMe($row_cate[0]);";
                        ...
                        <div style="width:400px;margin-left: 300px;">
                                ID/Keyword：<input type="text" id="keyword" name="keyword" value="<?=$_REQUEST['keyword']?>" />
                                <input value="search" type="button" onclick="locationurl();" /><br />
                            <?php
                            if(!empty($_REQUEST['keyword'])) {
...
//                                $sql="SELECT premade_id, title , description FROM ost_kb_premade WHERE isenabled=1 AND (title like '%".$_POST['keyword']."%' or description like '%".$_POST['keyword']."%' or answer like '%".$_POST['keyword']."%')";
                                ...
                                if($canned && db_num_rows($canned)) {
                                    while(list($cannedId,$title,$description)=db_fetch_row($canned)) {
                                        //'.$_SERVER['PHP_SELF']."?id=".$id."&premadeId=".$cannedId.'&keyword='.$_REQUEST['keyword'].'
                                        echo '<a href="javascript:addMe('.($cannedId*10000).');"><img src="images/page.png" />&nbsp;&nbsp;'.$description.'</a><br />';
                                    }
                                }
                            }
                            ?>
        ...
        <!--
            ...
            <?}?>
        -->
            <textarea name="issue" id="issue" cols="55" rows="8" wrap="soft"><?=$info['issue']?></textarea></td>
    </tr>
        ...
        <td> <?php
            ...
            <div style="margin-top: 2px;">
                ...
                <input type="hidden" name="dept_sign" id="dept_sign" value="<?=$row_dept[0]?>" />
                            <script type="text/javascript">
                                function use_sign(id)
                                {
                                    ...
                                    document.getElementById('issue').value = document.getElementById('issue').value + con;
                                }
                            </script>
                ...
                <?if($appendStaffSig) {?>
                 ...
                 <?}?>
                 <label><input type="radio" name="signature" value="dept" <?=$info['signature']=='dept'?'checked':''?> onclick="use_sign('dept_sign')" > Dept Signature (if any)</label>
            </div>
        </td>
    </tr>
    <tr height=2px><td align="left" colspan=2 >&nbsp;</td></tr>
