            ...
            <div class="tabber">
                <div id="reply" class="tabbertab" align="left">
                    ...
                    <div style="width:500px;padding-left: 200px;">
                        <form id="keyfrom" method="post" action="<?='tickets.php?id='.$_GET['id']?>">
                            ID/Keyword：<input type="text" name="keyword" value="<?=$_REQUEST['keyword']?>" />
...
//                        $cid=$_GET['classId'];
                        if(isset($cid)) {
                            echo "<script>window.location.hash='#dtree';</script>";
                        }
                        ...
                        if($class && db_num_rows($class)) {
                            ?>
                        <div id="dtree" class="dtree" style="float:left;">
                            <p><a href="javascript: d.openAll();">open all</a> | <a href="javascript: d.closeAll();">close all</a></p>
                                ...
                                while(list($classId,$name,$parentId)=db_fetch_row($class)) {
                                    $tempUrl=$_SERVER['PHP_SELF']."?id=".$id."&classId=".$classId;
                                    $tempTreeStr=$tempTreeStr."d.add($classId,$parentId,'&nbsp;$name','$tempUrl');";
                                    //搜索种类下的模版
                                    $result_cate = db_query('select p.* from ost_kb_premade p,ost_premade_category c where p.premade_id = c.premade_id and c.category_id='.$classId);
                                    while($row_cate = db_fetch_row($result_cate)) {
                                        $row_cate[0] = $row_cate[0]*10000;
                                        $tempUrl=$_SERVER['PHP_SELF']."?id=".$id."&premadeId=".$row_cate[0];
                                        $tempTreeStr=$tempTreeStr."d.add(".$row_cate[0].",$classId,'"."&nbsp;".$row_cate[4]."','$tempUrl');";
                                    }
                                }
                        ...
                        <div style="width:400px;margin-left: 300px;">
                            <?php
                            if(!empty($_REQUEST['keyword'])) {
                                $sql="SELECT premade_id, title , description FROM ost_kb_premade WHERE isenabled=1 AND (description like '%".$_REQUEST['keyword']."%' or no = '".$_REQUEST['keyword']."')";
//                                $sql="SELECT premade_id, title , description FROM ost_kb_premade WHERE isenabled=1 AND (title like '%".$_POST['keyword']."%' or description like '%".$_POST['keyword']."%' or answer like '%".$_POST['keyword']."%')";
                                $canned=db_query($sql);
                                if($canned && db_num_rows($canned)) {
                                    while(list($cannedId,$title,$description)=db_fetch_row($canned)) {
                                        $cannedId=$cannedId*10000;
                                        echo '<a href="'.$_SERVER['PHP_SELF']."?id=".$id."&premadeId=".$cannedId.'&keyword='.$_REQUEST['keyword'].'"><img src="images/page.png" />&nbsp;&nbsp;'.$description.'</a><br />';
                                    }
                                }
                            }
                            ?>
                        ...
                        <div>
                            ...
                            /*
                            if($cid>0) {
                                $sql='SELECT ost_kb_premade.premade_id, ost_kb_premade.title ,ost_kb_premade.description FROM ost_kb_premade INNER JOIN	ost_premade_category ON ost_premade_category.premade_id = ost_kb_premade.premade_id INNER JOIN ost_category ON ost_premade_category.category_id = ost_category.category_id WHERE isenabled=1 '.' AND (dept_id=0 OR dept_id='.db_input($ticket->getDeptId()).') AND ost_category.category_id ='.$cid;
                            ...
                            if($canned && db_num_rows($canned)) {
                                ?>
                            Canned Response:&nbsp;
                            <select id="canned" name="canned" onChange="getCannedResponse(this.options[this.selectedIndex].value,this.form,'response');this.selectedIndex='0';" >
                                <option value="0" selected="selected">Select a premade reply</option>
                                    <?while(list($cannedId,$title,$description)=db_fetch_row($canned)) { ?>
                                <option value="<?=$cannedId?>" ><?=$cannedId?>:<?=Format::htmlchars($description)?></option>
                                        <?}?>
                            ...
                            */
                            if($_GET['premadeId']>0) {
                                $result_premade = db_query('select * from ost_kb_premade where premade_id='.($_GET['premadeId']/10000));
                                $row_premade = db_fetch_row($result_premade);
                                $info['response'] .= $row_premade[5];
                                //署名
                            }

                            //个人署名
                            $result_user = db_query("select signature,dept_id from ost_staff where username ='".$_SESSION['_staff']['userID']."'");
                            $row_user = db_fetch_row($result_user);
                            //部门署名
                            $result_dept = db_query("select dept_signature from ost_department where dept_id =".$row_user[1]);
                            $row_dept = db_fetch_row($result_dept);
                            ?>
                            <input type="hidden" name="user_sign" id="user_sign" value="<?=$row_user[0]?>" />
                            <input type="hidden" name="dept_sign" id="dept_sign" value="<?=$row_dept[0]?>" />
                            <script type="text/javascript">
                                function use_sign(id)
                                {
                                    var con = document.getElementById(id).value;
                                    document.getElementById('response').value = document.getElementById('response').value + con;
                                }
                            </script>
                        ...
                        <div style="margin-top: 10px;">
                            ...
                            <label><input type="radio" name="signature" value="none" checked > None</label>
                                <?if($appendStaffSig) {?>
                            <label> <input type="radio" name="signature" value="mine" <?=$info['signature']=='mine'?'checked':''?> onclick="use_sign('user_sign')" > My signature</label>
                                    <?}?>
                                <?if($appendDeptSig) {?>
                            <label><input type="radio" name="signature" value="dept" <?=$info['signature']=='dept'?'checked':''?> onclick="use_sign('dept_sign')" > Dept Signature</label>
                                    <?}?>
                        </div>
                            <?}?>
                        <div style="margin-top: 3px;">
