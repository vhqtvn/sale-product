        ...
        case 'add':
            ...
            if(!$errors) {
                $sql=' SET updated=NOW(),isenabled='.db_input($_POST['isenabled']).
                        ...
                        ', no='.db_input(substr($_POST['no'],0,4)).
                        ', title='.db_input(Format::striptags($_POST['title'])).
                        ', description='.db_input(Format::striptags($_POST['description'])).
...
//                    $category=$_POST['category'];
                    ...
                    foreach ($category_ids as $category_id) {
                        if($category_id>0) {
                            $sql4category='INSERT INTO ost_premade_category SET premade_id='.$idid.' , category_id='.$category_id;
                            $res=db_query($sql4category);
                        }
                    }
                     ...
                     * 先全部删除掉
                     */
                    db_query('delete from ost_premade_category where premade_id='.db_input($_POST['id']));
                    foreach ($category_ids as $category_id) {
                        if($category_id>0) {
                            $sql4category='INSERT INTO ost_premade_category SET premade_id='.$idid.' , category_id='.$category_id;
                            $res=db_query($sql4category);
...
$inc=$page?$page:'smart.premade.inc.php';

$nav->setTabActive('kbase');
$nav->addSubMenu(array('desc'=>'Premade Replies','href'=>'smart.kb.php','iconclass'=>'premade'));
$nav->addSubMenu(array('desc'=>'New Premade Reply','href'=>'smart.kb.php?a=add','iconclass'=>'newPremade'));

//echo STAFFINC_DIR.$inc ;

require_once(STAFFINC_DIR.'smart.header.inc.php');
require_once(STAFFINC_DIR.$inc);
require_once(STAFFINC_DIR.'smart.footer.inc.php');
