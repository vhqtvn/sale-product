...
<head>
   ...
   <?php
		...
		$purchaseTaskId = $params['arg1'] ;

		$purchaseProductId = $params['arg1'] ;
		
		$SqlUtils  = ClassRegistry::init("SqlUtils") ;
		$purchaseProduct = $SqlUtils->getObject( 'sql_purchase_new_productsForPrint',array('purchaseProductId'=>$purchaseProductId)) ;
		...
		*/
    	...
    	$printTime = empty( $purchaseProduct['IN_PRINT_TIME'])? $currentTime: $purchaseProduct['IN_PRINT_TIME'] ;
	?>
	...
	$( function(){
			$(".print-btn").click(function(){
					if( window.confirm("确认打印采购入库单？") ){
						$.dataservice("model:NewPurchaseService.savePrintTime",{'inPrintTime':'<?php echo $printTime;?>',purchaseProductId:'<?php echo $purchaseProductId;?>'}, function(){
							$(".print").hide() ;
							window.print() ;
							},{noblock:true});
					}
...
<body>
	...
	<div style="padding:5px 10px;font-size:13px;font-weight:bold;">
	   ...
	   <div class="span5">
			采购任务编号：<?php echo $purchaseProduct['CODE']?>
		</div>
		<div class="span3">
			采购负责人：<?php echo $purchaseProduct['EXECUTOR_NAME']?>
		</div>
		<div class="span4">
