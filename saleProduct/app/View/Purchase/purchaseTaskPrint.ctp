	...
	$( function(){
			$(".print-btn").click(function(){
					if( window.confirm("确认打印采购确认单？") ){
						$.dataservice("model:NewPurchaseService.savePrintTime",{'printTime':'',purchaseProductId:'<?php echo $purchaseProduct['ID'];?>'}, function(){
							$(".print").hide() ;
							window.print() ;
						},{noblock:true});
					}
		...
		<tr>
			...
			<td style="text-algin:center;">
				...
				if(empty($providor)){
					...
					$qtc = $pd['QTC'] ;
					if( empty($qtc) ){
						$qtc = $providor.'-'.$purchaseProduct['S_NO'].'-'.$sku ;
						
						$qtc = str_replace("SUP-00", "", $qtc) ;
						$qtc = str_replace("SUP-0", "", $qtc) ;
						$qtc = str_replace("-10000", "-", $qtc) ;
						
						$SqlUtils->exeSql("update sc_purchase_product set qtc = '{@#qtc#}' where id='{@#purchaseProductId#}'",
								array("qtc"=>$qtc,"purchaseProductId"=>$pd['REQ_PRODUCT_ID'])) ;
					}
				}
