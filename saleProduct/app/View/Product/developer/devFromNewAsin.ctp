			...
			$(".btn-save").click(function(){
				if( !$.validation.validate('#personForm').errorInfo ) {
					if(window.confirm("确认保存吗？")){
						...
						$.dataservice("model:NewProductDev.addNewAsinDev",json,function(result){
							if(result){
								var items = [] ;
								$(result).each(function(){
									var devId = this.split("||")[0] ;
									var asin = this.split("||")[1] ;
									//http://localhost/saleProduct/index.php/page/forward/Product.developer.edit_product_dev/8
									items.push("<a target='_blank' href='"+contextPath+"/page/forward/Product.developer.edit_product_dev/"+devId+"'>"+asin+"</a>") ;
								}) ;
