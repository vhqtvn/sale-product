			...
			$.each(marketApps,function(i,category){			
				...
				appsHtml += "<div class='catNavTrigger' appid='"+category.id+"' appcode='"+category.code+"'>";
				 ...
				 if(appsIco.isClass ){
					...
					appsHtml += "<div class='cat_icon' style='background:url(http://boss.bingocc.com/product/download?filepath="+appsIco.value+") no-repeat;'></div>";		
				 };
			   ...
			   $(this).removeClass("hover"); 
				...
				webosService.loadAppStoreApps("-" , appId , appCode ,function(functions){
					...
					$(functions).each(function(i,app){
							...
							appDesc = (app.briefDesc||"").substring(0, 125), // 应用的描述
							appImgs = app.thumbnail||"", // 应用的截图
							appPublisher = app.publisher||"", //应用的发布者
							appVersion=app.version||"",
							appCreateTime=app.createTime||"",
							appPubdate = app.pubdate||""; //应用的发布时间
							if((app.briefDesc||"").length>125){
								appDesc+="...";
							}
							var time=new Date();
							time.setTime(appCreateTime);
							appCreateTime=time.getUTCFullYear()+"-"+time.getUTCMonth()+"-"+time.getUTCDate();
							console.log(appCreateTime);
							var	appsList = "";
							...
							if(appIcon.isClass ){
								...
								//appsList += "<div class='column1'><div class='appIcon' style='background:url(http://boss.bingocc.com/product/download?filepath="+appIcon.value+") no-repeat;'></div></div>";								
								appsList += "<div class='column1'><img class='appIcon' src='http://boss.bingocc.com/product/download?filepath="+appIcon.value+"'/></div>";								
							};
							
							appsList += "<div class='column2'>";
							appsList += "<span class='appName'><a href='javascript:'>"+appName+"</a></span>";
							appsList += "<span class='appVersion'>版本："+appVersion+"</span>";
							appsList += "<span class='appVersion'>创建时间："+appCreateTime+"</span>";
							...
							if(appDesktop == '1'){
								...
								//appsList += "<input type='checkbox' value='"+appID+"' id='"+appID+"_seeall'><label for='"+appID+"_seeall'>所有人可见</label>";
							}
							appsList += "</div>";
							appsList += "</li>";
	...
	//绑定列表的点击事件
	 function bind_list_click(){
		...
		$('.appListUl li .column1,.appListUl li .column2').click(function(){
			...
			App.loadView("appdetail.html?prdtId=" + appId, function(view) {
				$("#all_apps").html("");
	...
	function bing_list_add(){
		...
		$('.appAddBtn').click(function(e){
				...
				webosService.installApp(appDID,seeall,function(){
				    ...
				    getMarketApp(appDID).visibleAll = seeall?"visible":"" ;
