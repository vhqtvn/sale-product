...
<head>
   ...
   $(function(){
		...
		$("[name='quantity']").blur(function(){
			...
			var realId = json.realId ;
			statusRender(realId) ;
		}).trigger("blur") ;

		function statusRender(realId){
			//信息显示
			var json = $("."+realId+"-data").toJson() ;
			$("."+realId+"-log").hide() ;
			var quantity = parseInt( json.quantity||0) ;
			var realNum = parseInt( json.realNum||0) ;
			var reqNum = parseInt( json.reqNum||0) ;
			if(  quantity > realNum ){
				$("."+realId+"-log").show().find("td").html("入库数量超过最大库存数！") ;
				return ;
			}

			if( quantity > reqNum ){
				$("."+realId+"-log").show().find("td").html("入库数量大于需求数量！") ;
				//return ;
			}

			if(  reqNum >  realNum ){
				$("."+realId+"-log").show().find("td").html("需求数量大于实际库存数量！") ;
				//return ;
			}
			//需求状态  
			var _quantity = quantity ;
			$("."+realId+"-row").find(".req-row").each(function(){
					var _j = $(this).toJson() ;
			...
			var _quantity = quantity ;
			//锁定库存状态
			$("."+realId+"-row").find(".inventory-row").each(function(){
				var _j = $(this).toJson() ;
		...
		$(".btn-save").click(function(){
			...
			if( row.length >0  ){
				...
				json.inId = inId ;
				json.items = row ;
				$.dataservice("model:Warehouse.In.doSaveBoxProductForReq",json,function(result){
					//window.close() ;
				});
			}
			return false ;
...
<body class="container-popup">
	...
	foreach($reqProductList as $product){ 
		...
		//获取可以发货到目标仓库的库存明细
		$inventoryItemList = $SqlUtils->exeSqlWithFormat("sql_supplychain_inventoryItem_canToWarehouse",
						array('sourceWarehouseId'=>$warehoseIn['SOURCE_WAREHOUSE_ID'],"realId"=>$product['ID'])) ;


		//获取需求明细
		$reqProductItemList = $SqlUtils->exeSqlWithFormat("sql_supplychain_requirement_listFBMItemByWarehouseId",
					array('warehouseId'=>$warehoseIn['WAREHOUSE_ID'],"realId"=>$product['ID'])) ;
		...
		?>
					...
					<tr class="check-product-row-1">
						...
						<td>
								<?php echo $reqQuantity ?>
						</td>
						<td>
								<?php echo $realNum;?>(其中自由库存<?php echo $freeNum;?>)&nbsp;&nbsp;
								<a href="#"  pid='<?php echo $product['ID'];?>'  class="view-detail">明细</a>
						</td>
					</tr>
					<tr class="check-product-row hide  <?php echo $product['ID'];?>-data"  >
					...
					<tr class="hide  errorLog  <?php echo $product['ID'];?>-log">
					   	<td  colspan="5"  class="alert alert-danger" style="padding:2px;height:18px;">
					   			实际库存总量小于需求库存(需求总库存【<?php echo $reqQuantity;?>】,实际库存【<?php echo $realNum;?>】)
					    </td>
					 </tr>
					<tr class="rk-product-row  hide  <?php echo $product['ID'];?>-row">
					   	<td  colspan="5">
