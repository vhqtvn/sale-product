...
class Utils extends AppModel {
	...
	public function echoTreeScript( $categorys, $selected , $callback ) {
		$index = 0;
		foreach ( $categorys as $Record ) {
			$sfs = $this->formatObject ( $Record );
			$id = $sfs ['ID'];
			$callback( $sfs, $index ,$selected) ;
	...
	public function _processRowCompetetion($e, $details, $type, $base, $numType,$keyMaps=null) {
		...
		//print_r($sellerName) ;
			...
			$index ++;
			$details [] = array ("SELLER_NAME" => $sellerName, "SELLER_URL" => $sellerUrl, "SELLER_PRICE" => $price, "SELLER_IMG" => $sellerImg, "SELLER_SHIP_PRICE" => $priceShipping, "TYPE" => $type . $index, "PER_POSITIVE" => $prePositive, "TOTAL_RATING" => $totalRating, "COUNTRY" => $country );
		}
