	$(function(){
			var sqlId = "sql_sc_order_list_one" ;
...
function renderAction(index){
	$(".save-btn").show() ;
	if(index == 0){//未审核订单
		currentQueryKey = "sql_sc_order_list_one" ;
		$(".grid-content").llygrid("reload",{pickStatus:'',status:5,sqlId:"sql_sc_order_list_one"},true) ;
	}else if(index == 1){//合格订单
		currentQueryKey = "sql_sc_order_list_many" ;
		$(".grid-content").llygrid("reload",{pickStatus:'',status:5,sqlId:"sql_sc_order_list_many"},true) ;
	}else if(index == 2){//合格订单
		$(".grid-content").llygrid("reload",{pickStatus:'',status:5,sqlId:"sql_sc_order_list"},true) ;
		currentQueryKey = "sql_sc_order_list" ;
	}else if(index == 3){//风险客户
		$(".grid-content").llygrid("reload",{pickStatus:'',status:2,sqlId:"sql_sc_order_list"},true) ;
		currentQueryKey = "sql_sc_order_list" ;
	}else if(index == 4){//待退单
		$(".grid-content").llygrid("reload",{pickStatus:'',status:3,sqlId:"sql_sc_order_list"},true) ;
		currentQueryKey = "sql_sc_order_list" ;
	}else if(index == 5){//外购订单
		$(".grid-content").llygrid("reload",{pickStatus:'',status:4,sqlId:"sql_sc_order_list"},true) ;
		currentQueryKey = "sql_sc_order_list" ;
	}else if(index == 6){//加急单
		$(".grid-content").llygrid("reload",{pickStatus:'',status:6,sqlId:"sql_sc_order_list"},true) ;
		currentQueryKey = "sql_sc_order_list" ;
	}else if(index == 7){//特殊但
		$(".grid-content").llygrid("reload",{pickStatus:'',status:7,sqlId:"sql_sc_order_list"},true) ;
		currentQueryKey = "sql_sc_order_list" ;
	}
}
