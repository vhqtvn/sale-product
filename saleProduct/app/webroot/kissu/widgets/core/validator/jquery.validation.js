...
(function($) {
	...
	$.fn.validation = function(settings) {
		...
		if (settings.inlineValidation == true) { // Validating Inline ?
			...
			function _inlinEvent(caller,eventType) {
				$.validation.settings = settings;
		...
		$(this).find(validtorAttr).each(function(){
			if( $(this).attr(validtorAttrValue).indexOf("required")!=-1 ){
				if( $(this).parents(".control-group")[0] ){
					...
					if($(this).parents("td").prev()
						.find(".required-star")[0] ) return ;
					$(this).parents("td").prev()
						.find("label")
						.prepend("<span class='required-star'>*</span>") ;
				}	
			}
	...
	$.validation = {
		...
		buildPrompt : function(caller, promptText) { // ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
			...
			if( $(caller).parents("._td_layout")[0] || !$(caller).parents(".control-group")[0]   ){
					$(caller).attr("errorInfo", promptText).parents("td").removeClass("success").addClass("_td_layout control-group error").append("<span class='tipsy-container tipsy-error'></span>")  ;
					$(caller).attr("errorInfo", promptText).parents("td").prev().find("label").addClass("validator-error-label") ;
			}else{
		...
		closePrompt : function(caller) { // CLOSE PROMPT WHEN ERROR CORRECTED
			...
			//判断是否为table布局
			if( $(caller).parents("._td_layout")[0] || !$(caller).parents(".control-group")[0]   ){
				$(caller).parents("td").removeClass("error").addClass("_td_layout control-group success").append("<span class='tipsy-container tipsy-success'></span>") ; ;
				$(caller).removeAttr("errorInfo").parents("td").prev().find("label").removeClass("validator-error-label").addClass("validator-success-label") ;
			}else{
				$(caller).parents(".control-group").removeClass("error success") ;
	...
	/*	var rules = [{"FieldName":"Name123","ReplaceValidationMessageContents":true,"ValidationMessageId":"Name_validationMessage","ValidationRules":[{"ErrorMessage":"姓名长度超出限制。","ValidationParameters":{"min":20,"max":50},
	"ValidationType":"length"}]},

{"FieldName":"agreement","ReplaceValidationMessageContents":true,
	"ValidationMessageId":"LoginId_validationMessage",
	"ValidationRules":[{"ErrorMessage":"帐号已经存在，请选择其他帐号","ValidationParameters":
	{"url":"http://localhost:8080/Bingo-UI/statics/scripts/plugins/validator/demo/urlValidator.html",
		"additionalfields":"*.birthday,*.city"},"ValidationType":"remote"},
		{"ErrorMessage":"帐号长度超出限制。","ValidationParameters":{"max":30},"ValidationType":"length"},
		{"ErrorMessage":"帐号不允许为空。","ValidationParameters":{},"ValidationType":"required"}]},

		{"FieldName":"city","ReplaceValidationMessageContents":true,"ValidationMessageId":"Name_validationMessage","ValidationRules":[{"ErrorMessage":"姓名长度超出限制。","ValidationParameters":{"min":20,"max":50},
			"ValidationType":"length"}
			,{"ErrorMessage":{min:0,max:30},"ValidationParameters":{},
		"ValidationType":"length"}]},

		{"FieldName":"email","ValidationRules":[{"ErrorMessage":"性别选择错误。",
			"ValidationParameters":{"pattern":"[M|F|U]"},"ValidationType":"regex"}]}
] ;*/
