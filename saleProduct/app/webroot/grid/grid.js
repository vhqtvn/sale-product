...
(function($){
	...
	function adjustWidth(target){
			...
			$(".lly-grid-2-body table tr",target).each(function(index){
				$(".lly-grid-1-body table tr:eq("+index+")",target).height($(this).outerHeight()) ;
			}) ;
	}
	...
	function reload(target , ps,isAdd){
		var qs = target.data("rquerys"); //isAdd?target.data("rquerys"):target.data("querys") ;
		var p = $.extend({}, qs ,ps) ;
			...
			"editor":{
				...
				bindEvent:function(col,grid){
					grid.find("input[name='editor_"+col.key+"']").unbind("keyup").bind("keyup",function(){
						...
						var rawval = $.trim($(this).attr("rawval")||"") ;
						if(val){
							$(this).addClass("lly-grid-cell-update") ;
						}else{
							$(this).removeClass("lly-grid-cell-update") ;
						}
					});
 ...
 function formatGridData(data){
		...
		var ret = {records: array,totalRecord:array.length} ;
		return ret ;
	   }
...
function openCenterWindow(URL,wndWidth,wndHeight){
	...
	var form    = "width=" + wndWidth + ",height=" + wndHeight + ",left=" + wndLeft + ",top=" + wndTop + ",resizable=yes,scrollbars=yes";
