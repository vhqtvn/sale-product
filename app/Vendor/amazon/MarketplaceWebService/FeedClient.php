...
define('CONVERTED_FEED_PARAMETERS_KEY', 'PARAMETERS');
define('CONVERTED_FEED_HEADERS_KEY', 'HEADERS');
 ...
 *
 */
class MarketplaceWebService_FeedClient implements MarketplaceWebService_Interface
{
  ...
  private function invoke(array $converted, $dataHandle = null, $contentMd5 = null)
  {
  	...
  	$parameters = $converted[CONVERTED_FEED_PARAMETERS_KEY];
    ...
    if (empty($this->config['ServiceURL'])) {
      ...
      $converted[CONVERTED_FEED_PARAMETERS_KEY] = $parameters;
  ...
  private function performRequest($action, array $converted, $dataHandle = null, $contentMd5 = null) {
    ...
    $this->errorResponseBody = @fopen('php://temp', 'rw+');
  ...
  private function responseCallback($ch, $string) {
  	...
  	if ($httpStatusCode == 200) {
  		...
  		$s = fwrite($responseHandle, $string);
  		
  		  $dom = new DOMDocument();
		  $dom->loadXML($string);
		  echo '<br>---------------------------------<br>' ;
		  $array = $this->getArray($dom->documentElement) ;
		  
		  if( isset($array['Message']) && isset($array['Message'][0]) && isset($array['Message'][0]['ProcessingReport'])  ){
		  	 $feedId = $array['Message'][0]['ProcessingReport'][0]['DocumentTransactionID']['0']['#text'] ;
			  $Status = $array['Message'][0]['ProcessingReport'][0]['StatusCode']['0']['#text'] ;
			  $sucess = $array['Message'][0]['ProcessingReport'][0]['ProcessingSummary'][0]['MessagesSuccessful']['0']['#text'] ;
			  $error  = $array['Message'][0]['ProcessingReport'][0]['ProcessingSummary'][0]['MessagesWithError']['0']['#text'] ;
	  		  
	  		   $amazonAccount  = ClassRegistry::init("Amazonaccount") ;
	  		   $amazonAccount->updateAccountProductFeed(array(
	  		   	"feedId"=>$feedId,
	  		   	"status"=>$Status,
	  		   	"sucess"=>$sucess,
	  		   	"error"=>$error,
...
function getArray($node) {
  ...
  private function configureCurlOptions($action, array $converted, $streamHandle = null, $contentMd5 = null) {
    ...
    if ($requestType == RequestType::POST_UPLOAD) {
      ...
      $serviceUrl .= '?' . $this->getParametersAsString($converted[CONVERTED_FEED_PARAMETERS_KEY]);
      ...
      $curlOptions[CURLOPT_HTTPHEADER] = array_merge($header, $converted[CONVERTED_FEED_HEADERS_KEY]);
      ...
      $curlOptions[CURLOPT_POSTFIELDS] = $this->getParametersAsString($converted[CONVERTED_FEED_PARAMETERS_KEY]);
  ...
  private function getFormattedTimestamp($dateTime) {
    ...
    private function convertGetReport($request) {
      ...
      return array(CONVERTED_FEED_PARAMETERS_KEY => $parameters, CONVERTED_FEED_HEADERS_KEY => $this->defaultHeaders);
    }
	...
	$marketplaceIdList = $request->getMarketplaceIdList();
      ...
      return array(CONVERTED_FEED_PARAMETERS_KEY => $parameters, CONVERTED_FEED_HEADERS_KEY => $headers);
    }
